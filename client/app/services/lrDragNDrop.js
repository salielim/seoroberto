(function(ng) {
    ...
    function(document) {
      ...
      this.get = function(namespace) {
        ...
        if (modelItem) {
          ...
          return modelItem.safe === true
            ? modelItem.item
            : modelItem.collection.splice(itemIndex, 1)[0];
        } else {
          return null;
        }
      };
  ...
  function parseRepeater(scope, attr) {
    var repeatExpression = attr.ngRepeat,
      match;
  ...
  module.directive("lrDragSrcSafe", [
    "lrDragStore",
    function(store) {
      return {
        ...
        link: function(scope, element, attr) {
          ...
          function isAfter(x, y) {
            ...
            return (
              element[0].offsetHeight -
                x * element[0].offsetHeight / element[0].offsetWidth <
              y
            );
          }
          ...
          element.bind("dragover", function(evt) {
            ...
            if (store.isHolding(key)) {
              className = isAfter(evt.offsetX, evt.offsetY)
